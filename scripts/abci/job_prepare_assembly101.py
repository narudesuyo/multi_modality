"""ABCI job submission script for prepare_assembly101_clips.py (Step 2).

Generates a PBS script that runs ``prepare_assembly101_clips.py`` on an ABCI
GPU node and submits it via ``qsub``.

Usage::

    # Preview
    python scripts/abci/job_prepare_assembly101.py --dry-run

    # Submit train split (default)
    python scripts/abci/job_prepare_assembly101.py

    # Submit validation split
...
class PrepareAssembly101Config:
    """ABCI PBS job configuration for prepare_assembly101_clips.py."""

    # PBS queue name (GPU needed for SMPL-H forward kinematics).
    queue: str = "rt_HG"
    # ABCI project group.
    ...
    # Device for SMPL-H forward kinematics.
    device: str = "cuda"

    # ---- Data paths ----
    # DATA_ROOT for dataset paths.
    data_root: str = "/groups/gch51606/takehiko.ohkawa/tmp_data"
    # SMPL-H model directory (containing smplh/SMPLH_NEUTRAL.npz).
...
def render_pbs_script(config: PrepareAssembly101Config, job_name: str) -> str:
    ...
    # --- prepare_assembly101_clips.py args ---
    script_args = f"--split {config.split} --device {config.device}"
    if config.skip_existing:
        script_args += " --skip-existing"
    if config.smplh_model_path:
        script_args += f" --model-path {config.smplh_model_path}"
    ...
    body = textwrap.dedent(f"""\
        ...
        # ---- Run prepare_assembly101_clips.py ----
        echo "[INFO]: Running: python scripts/pretraining/stage2/1B_motion/prepare_assembly101_clips.py {script_args}"
        python scripts/pretraining/stage2/1B_motion/prepare_assembly101_clips.py {script_args}
...
def main() -> None:
    ...
    config = tyro.cli(PrepareAssembly101Config)

    # Generate job name
    now = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    job_name = config.job_name or f"prep-asm101-{config.split}-{now}"
